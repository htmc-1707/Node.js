...
function ProductList() {
    ...
    // Hàm để tìm tất cả danh mục con
    const findChildCategories = (categoryId) => {
        let childCategories = categories.filter(category => category.parentId === categoryId);
        let allChildren = [...childCategories];
        childCategories.forEach(child => {
            allChildren = [...allChildren, ...findChildCategories(child.id)];
        });
        return [categoryId, ...allChildren.map(child => child.id)]; // Bao gồm cả danh mục cha
    };

    useEffect(() => {
        if (selectedCategory) {
            const allCategoryIds = findChildCategories(selectedCategory);
            setFilteredProducts(products.filter(product => allCategoryIds.includes(product.categoryId)));
        } else {
                                    ...
                                    {product.salePrice && (
                                        <div className="sale-star">
                                            <div className="sale-starr">
                                                {product.salePrice < product.price ? "HOT" : ""}
                                            </div>
                                        </div>
                                    )}
                                    <style>
